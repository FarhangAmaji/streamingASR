...
class TranscriptionOutputHandler:
    ...
    def _calculateSegmentLoudness(self, audioData):
        ...
        if audioData.dtype.kind != 'f':
             ...
             if audioData.dtype.kind in ('i', 'u'):
                 ...
                 if max_val > min_val:
                      audioData = (audioData.astype(np.float32) - min_val) / (max_val - min_val) * 2.0 - 1.0
                 else:
    ...
    def processTranscriptionResult(self, transcription, audioData):
        ...
        shouldOutput, finalText = self._filterAndFormatTranscription(transcription, segmentLoudness, audioData)
    ...
    def _filterAndFormatTranscription(self, transcription, segmentLoudness, audioData):
        ...
        # Only perform if loudness is available (segmentLoudness != -1)
        # and audioData is available (checked within the function)
        ...
        if self.config.get('removeTrailingDots', True): # Default to True if not set
            # Remove trailing dots and spaces robustly
            formattedText = formattedText.rstrip('. ')
        formattedText = formattedText.lstrip(" ") # Remove leading spaces

        if not formattedText:
            logDebug("Text became empty after final formatting steps.")
            return False, ""
    ...
    def _shouldSkipTranscriptionDueToSilenceOrLowContent(self, segmentMeanLoudness, audioData):
        ...
        if minLoudDuration > 0:
            ...
            if totalLoudDuration < minLoudDuration:
                logDebug(f"Silence skip CONFIRMED: Total loud duration ({totalLoudDuration:.2f}s) < min ({minLoudDuration:.2f}s). (Avg Loudness: {segmentMeanLoudness:.6f})")
                return True
            # else:
        ...
        logDebug(f"Segment passed min loud duration but mean loudness ({segmentMeanLoudness:.6f}) < skip threshold ({silenceSkipThreshold:.6f}). Checking overrides...")

        # Check Beginning
        if checkLeadingSec > 0:
            leadingSamples = min(int(checkLeadingSec * sampleRate), len(audioData)) # Avoid over-indexing
            if leadingSamples > 0:
                ...
                if leadingLoudness >= chunkSilenceThreshold:
                    logDebug(f"Silence skip OVERRIDDEN: Leading {checkLeadingSec:.2f}s loud enough ({leadingLoudness:.6f}).")
                    return False # DO NOT SKIP

        # Check Trailing
        if checkTrailingSec > 0:
            trailingSamples = min(int(checkTrailingSec * sampleRate), len(audioData)) # Avoid over-indexing
            if trailingSamples > 0:
                ...
                if trailingLoudness >= chunkSilenceThreshold:
                    logDebug(f"Silence skip OVERRIDDEN: Trailing {checkTrailingSec:.2f}s loud enough ({trailingLoudness:.6f}).")
                    return False # DO NOT SKIP

        # --- 4. Final Decision (Low Avg, No Overrides) ---
        logDebug(f"Silence skip CONFIRMED: Low avg loudness ({segmentMeanLoudness:.6f}) and no start/end overrides triggered.")
        return True # SKIP
    ...
    def _isFalsePositive(self, cleanedText_lower, segmentLoudness):
        ...
        if checkText in commonFalseWords_normalized:
            loudnessThreshold = self.config.get('loudnessThresholdOf_commonFalseDetectedWords', 0.0008)
            if segmentLoudness < loudnessThreshold:
                logDebug(f"'{checkText}' IS false positive (Loudness {segmentLoudness:.6f} < {loudnessThreshold:.6f}). Filtering.")
                return True
            else:
                logDebug(f"'{checkText}' matches false positive BUT loudness ({segmentLoudness:.6f}) >= threshold. Not filtering.")
    ...
    def _handleValidOutput(self, finalText):
        ...
        if self.stateManager.isOutputEnabled():
             ...
             if not self.systemInteractionHandler.isModifierKeyPressed("ctrl"):
                 self.systemInteractionHandler.typeText(finalText + " ") # Add space after output
             else:
    ...
    def transcribeFile(self, audioFilePath, outputFilePath=None):
        ...
        try:
            ...
            if not self.asrModelHandler.isModelLoaded():
                logInfo(f"ASR model ({handlerType}) not ready for file transcription, attempting to load/prepare...")
                load_success = self.asrModelHandler.loadModel() # Trigger load/check
                if not load_success:
            ...
            if not self.asrModelHandler.isModelLoaded():
                logError(f"Model ({handlerType}) still not loaded after load attempt. File transcription aborted.")
                return None
            ...
            try:
                ...
                logError("Hint: Ensure the file is a valid audio format (WAV, FLAC, OGG, etc.) and not corrupted.")
                return None

            fileDuration = len(audioData) / sampleRate if sampleRate > 0 else 0
            logInfo(f"Audio file read successfully: {audioFilePath.name} (Sample Rate: {sampleRate}Hz, Duration: {fileDuration:.2f}s)")
