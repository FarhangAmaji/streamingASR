...
def configureLogging(logFileName='mainService.log', debugMode=False):
    ...
    # --- File Handler ---
    try:
        fileHandler = logging.FileHandler(logFileName, mode='a', encoding='utf-8') # Append mode, UTF-8
        ...
        print(f"CRITICAL ERROR: Could not configure file logging to '{logFileName}': {e}", file=sys.stderr)
    ...
    logger.info(f"Logging configured. Console Level: {logging.getLevelName(logLevel)}. File: '{logFileName}'")
...
def convertWindowsPathToWsl(windowsPath) -> str | None:
    ...
    try:
        ...
        if not windowsPath.is_absolute():
            ...
            logWarning(f"Path '{windowsPath}' is not absolute. Attempting resolution to '{resolvedPath}' for WSL conversion.")
        ...
        # Handle basic UNC paths (\\server\share\...)
            ...
            logWarning("Attempting basic UNC path conversion for WSL - might not work depending on WSL mount configuration.")
